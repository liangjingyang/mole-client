...
%% p2p data
handle_info({udp, _Socket, Ip, Port, <<?HEART_BEAT:8, Packet/binary>>}, State) ->
...
handle_info(server_req, State) ->
    case State#state.his_net of
        undefined ->
            case State#state.conn_type of
                undefined ->
                    erlang:send_after(3000, self(), server_req),
...
handle_info(heartbeat, State) ->
    ...
    gen_udp:send(State#state.socket, Ip, Port, <<?HEART_BEAT:8>>),
    {noreply, State};
...
key_to_128(Key) ->
    ...
    case Size < 128 of
        true ->
            B = lists:foldl(fun(_, Acc) ->
                        <<$$:8, Acc/binary>>
                end, <<>>, lists:seq(1, ((128 div 8) - Size))),
            <<Key/binary, B/binary>>;
        false ->
